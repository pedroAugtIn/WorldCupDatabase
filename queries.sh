...
echo $($PSQL "SELECT teams_winner.name AS team_name FROM games LEFT JOIN teams AS teams_winner ON games.winner_id = teams_winner.team_id WHERE round = 'Eighth-Final' AND year = 2014 UNION SELECT teams_opponent.name AS team_name FROM games LEFT JOIN teams AS teams_opponent ON games.opponent_id = teams_opponent.team_id WHERE round = 'Eighth-Final' AND year = 2014 ORDER BY team_name;")
